...
@click.command()
@click.pass_context
@click.option('--file-name', '-n', help='导出文件名', prompt='请输入文件名', default='data', type=str)
...
def export(ctx, file_name: str, file_format: str):
    ...
    for volume in project_path.iterdir():
        ...
        volume_data['状态'] = volume_data['状态'].map(match_status)
        volume_data['主队'] = volume_data['主队'].map(team['名称'])
        volume_data['客队'] = volume_data['客队'].map(team['名称'])
