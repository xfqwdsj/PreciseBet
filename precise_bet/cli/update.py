...
def update(
    ...
    action: Annotated[
        Action,
        typer.Argument(
            help="要更新的数据类型", case_sensitive=False, parser=actions_parser
        ),
    ],
    ...
    interval: Annotated[
        int, typer.Option("--interval", "-i", help="基准更新间隔（秒）")
    ] = 5,
    extra_interval: Annotated[
        int, typer.Option("--extra-interval", "-e", help="额外更新间隔（秒）")
    ] = 60,
    extra_interval_probability: Annotated[
        float,
        typer.Option(
            "--extra-interval-probability", "-p", help="额外更新间隔概率（0-1）"
        ),
    ] = 0,
    interval_offset_range: Annotated[
        int,
        typer.Option("--interval-offset-range", "-r", help="更新间隔偏移量范围（秒）"),
    ] = 2,
    random_ua: Annotated[bool, typer.Option(help="随机 UA")] = False,
    last_updated_status: Annotated[
        str,
        typer.Option(
    ...
    handle_more_status_change: Annotated[
        bool, typer.Option(help="处理更多状态变化（如从中断类状态重新开始）")
    ] = True,
    break_hours: Annotated[
        int,
        typer.Option(
            "--break-hours",
            "-b",
            help="指定要跳过多少小时后的未开始的比赛（设为 0 以忽略，时）",
        ),
    ] = 6,
    only_new: Annotated[
        bool, typer.Option("--only-new", "-n", help="只更新从未获取过的比赛")
    ] = False,
    ...
    if volume_number is not None:
        global_data = global_data.loc[
            global_data[DataTable.volume_number] == volume_number
        ]
    ...
    last_updated_status_list: list[int] = [-2, -1] + init_status_list(
        last_updated_status
    )
    ...
    data = data.loc[
        data[MatchInformationTable.updated_match_status].isin(last_updated_status_list)
        | more_status_change
    ]
    ...
    data_analysis.loc["从未获取"] = len(
        data.loc[data[UpdatableTable.updated_time] == -1.0]
    )
    ...
    for status in data[DataTable.match_status].unique():
        data_analysis.loc[match_status_dict[status]] = len(
            data.loc[data[DataTable.match_status] == status]
        )
    ...
    rprint(
        f"使用额外更新间隔的概率 [bold blue]{extra_interval_probability}[/bold blue]，",
        end="",
    )
    rprint(
        f"更新间隔偏移量范围 [bold blue]{interval_offset_range}[/bold blue] 秒，",
        end="",
    )
    if len(last_updated_status_list) > 0:
        status_text_list = [
            match_status_dict[status]
            for status in list(set(last_updated_status_list) - {-1})
        ]
        rprint(
            f"只更新上次更新时状态为 [bold yellow]{status_text_list}[/bold yellow] 的比赛，",
            end="",
        )
    else:
    ...
    if len(status_list) > 0:
        ...
        rprint(
            f"只更新状态为 [bold yellow]{status_text_list}[/bold yellow] 的比赛，",
            end="",
        )
    else:
    ...
    if 0 in status_list and break_hours > 0:
        rprint(
            f"跳过 [bold blue]{break_hours}[/bold blue] 小时后的未开始的比赛，", end=""
        )
    ...
    with Progress(
        ...
        task = progress.add_task(
            f"正在更新{action.name}（按下 [bold]Ctrl[/bold] + [bold]C[/bold] 中断）...",
            total=len(data) + interval_sum,
        )
        ...
        for index, match_id in enumerate(data.index):
            rule(
                f"正在更新第 [yellow]{index + 1}[/yellow] / [blue]{len(data)}[/blue] 场比赛"
            )

            host_name = team_data.loc[
                global_data.loc[match_id, DataTable.host_id], TeamTable.name
            ]
            guest_name = team_data.loc[
                global_data.loc[match_id, DataTable.guest_id], TeamTable.name
            ]

            current_status = global_data.loc[match_id, DataTable.match_status]
            last_updated_status = data.loc[
                match_id, MatchInformationTable.updated_match_status
            ]
            current_status_text = (
                f"[bold blue]{match_status_dict[current_status]}[/bold blue]"
            )
            last_updated_status_text = (
                f"[bold yellow]{match_status_dict[last_updated_status]}[/bold yellow]"
            )
            status_text = f"比赛状态：{current_status_text}，上次更新时状态：{last_updated_status_text}"

            rprint(
                f"正在更新代号为 {match_id} 的比赛（{host_name} VS {guest_name}，{status_text}）的{action.name}信息..."
            )
