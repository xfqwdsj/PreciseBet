...
class Table(pd.DataFrame, ABC):
    ...
    def class_pairs(cls) -> dict[str, Column]:
        return {
            i: vars(cls)[i]
            for i in vars(cls)
            if not i.endswith("_") and isinstance(vars(cls)[i], Column)
        }
    ...
    def table_pairs(cls) -> OrderedDict[str, Column]:
        if cls._table_pairs is None:
            ...
            cls._table_pairs = OrderedDict[str, Column](
                sorted(result.items(), key=lambda x: x[1].order.value)
            )
    ...
    def create(self):
        super().__init__(
            pd.DataFrame(
                {
                    column: pd.Series(dtype=column.type)
                    for column in self.table_columns()
                }
            ).set_index(self.index_)
        )
        return self
...
class HandicapTable(MatchInformationTable):
    ...
    def empty_row(cls):
        return cls.generate_row(
            live_average_water1=None,
            live_average_handicap=None,
            live_average_water2=None,
            early_average_water1=None,
            early_average_handicap=None,
            early_average_water2=None,
            updated_time=-1.0,
            updated_match_status=-1,
        )
    ...
    def row_from_list(cls, handicap: list[float], updated_match_status: int):
        return cls.generate_row(
            live_average_water1=handicap[0],
            live_average_handicap=handicap[1],
            live_average_water2=handicap[2],
            early_average_water1=handicap[3],
            early_average_handicap=handicap[4],
            early_average_water2=handicap[5],
            updated_match_status=updated_match_status,
        ).append_updated_time()
...
class OddTable(MatchInformationTable, ABC):
    ...
    @classmethod
    def row_from_list(
        cls, odds: list[float], updated_time: float, updated_match_status: int
    ):
        return cls.generate_row(
            win=odds[0],
            draw=odds[1],
